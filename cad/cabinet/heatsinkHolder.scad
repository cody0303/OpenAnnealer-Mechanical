...
// Stands inside the cabinet on the feeder panel, its +X end face on the wall,
// centred between the coil's leads (z = 0 is the lead height). The heatsinks
// bolt to its sides with M4s into heat-set inserts.
...
assert(topSeat < holderLength - 5, "the top bolt's head sits almost out of the holder -- use a shorter topScrewLen");
...
//for the assembly: overall size
function heatsinkHolderSize() = [holderLength, heatsinkSpacing, 2*bossZ + mountingBossHeight];
...
module heatsinkHolder() {
    difference(){
        union(){
            ...
            cube([holderLength, 8, 2*bossZ], center=true);
            mirror_copy([0,0,1])
                translate([0,0,bossZ])
                    cube([holderLength, heatsinkSpacing, mountingBossHeight], center=true);
        }
        //top: the funnel's upper M5, straight through, counterbored from the inner end so the
        //head seats topSeat from the wall
        translate([holderLength/2, 0, bossZ])
            rotate([0,-90,0]) {
                bolt("M5", length=holderLength+1, kind="headless", length_clearance=1);
                translate([0, 0, topSeat]) cylinder(d=headCbD, h=holderLength);
            }
        mirror([0,0,1]){
            //bottom: an M5 in through the end face, its nut caught wallThickness behind it
            translate([holderLength/2-wallThickness, 0, bossZ])
                rotate([90,0,-90])
        ...
        translate([holderLength/2+1, 0, funnelHeadZ])
            rotate([0,-90,0])
                cylinder(d=10, h=6+1);
